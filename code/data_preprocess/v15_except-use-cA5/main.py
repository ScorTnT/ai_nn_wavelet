...
SAVE_DIR = os.path.join(BASE_PROJECT_DIR, 'wavelet_v15_2/train')
TRAINING_DIR = 'training'  # 'training' 'validation'
...
SEGMENT_LENGTH = 512  # 256, 512, 1024 등으로 변경 가능
SAMPLE_RATE = 2000
...
def create_feature_column_names():
    ...
    return columns
def filter_segments_by_cA5_features_only(segments_features):
    """
    cA5 레벨의 5개 특징만으로 필터링
    ...
    print(f"    cA5 레벨 특징만으로 필터링 (5개 특징)")
    
    # Set to store indices of segments that pass any cA5 feature filter
    selected_indices = set()
    
    # cA5 features are at indices 25-29 (6th level: cD1(0-4), cD2(5-9), cD3(10-14), cD4(15-19), cD5(20-24), cA5(25-29))
    cA5_feature_indices = [25, 26, 27, 28, 29]
    cA5_feature_names = ['cA5_mean_abs', 'cA5_mean_square', 'cA5_std', 'cA5_ratio', 'cA5_median']
    
    # For each cA5 feature, find segments that are >= mean for that feature
    for i, feature_idx in enumerate(cA5_feature_indices):
        ...
        print(f"      {cA5_feature_names[i]}: 평균={feature_mean:.6f}, "
    ...
    print(f"    cA5 특징 기반 중복 제거 후 최종 선택된 세그먼트: {len(filtered_features)}개 ({filter_ratio:.1f}% 유지)")
    
    return filtered_features

def process_heartbeat_dataset():
    """
    Process heartbeat dataset with cA5 feature-based filtering and save features to CSV files.
    """
    ...
    print("=== 심장박동 오디오 세그먼트 웨이블릿 특징 추출 (cA5 특징별 평균 필터링) ===")
    ...
    print(f"필터링 방식: cA5 레벨의 5개 특징만으로 평균 이상인 세그먼트 선택 후 중복 제거")
    ...
    for filename in sorted(wav_files):
        ...
        try:
            ...
            # Apply cA5 feature-based filtering
            filtered_features = filter_segments_by_cA5_features_only(segments_features)
    ...
    if all_data:
        ...
        csv_filename = f'wavelet_30_features_cA5_filtered_{SEGMENT_LENGTH}_{TRAINING_DIR}.csv'
        ...
        print(f"\n--- Dataset Statistics ---")
        print(f"Total segments (after cA5 filtering): {len(df)}")
        ...
        desc_filename = f'feature_description_cA5_filtered_{SEGMENT_LENGTH}.txt'
        feature_desc_path = os.path.join(SAVE_DIR, desc_filename)
        with open(feature_desc_path, 'w', encoding='utf-8') as f:
            f.write(f"30 Wavelet Features Description (cA5 Feature Filtered, Segment Length: {SEGMENT_LENGTH})\n")
            f.write("=" * 90 + "\n\n")
            ...
            f.write("Segmentation method: Fixed-length segments with cA5 feature-based filtering\n")
            f.write("- Audio files are divided into fixed-length segments\n")
            f.write(f"- Each segment is exactly {SEGMENT_LENGTH} samples long\n")
            f.write("- For each file, for each of the 5 cA5 features individually:\n")
            f.write("  1. Calculate the mean of that cA5 feature across all segments\n")
            f.write("  2. Select segments where that cA5 feature >= its mean\n")
            f.write("- Combine all selected segments and remove duplicates\n")
            f.write("- This keeps segments that excel in ANY cA5 feature\n")
            f.write("- cA5 features used: mean_abs, mean_square, std, ratio, median\n\n")
            ...
            f.write(f"\nDataset Summary:\n")
            f.write(f"Total segments (after cA5 filtering): {len(df)}\n")
