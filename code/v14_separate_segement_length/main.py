...
SAVE_DIR = os.path.join(BASE_PROJECT_DIR, 'wavelet_v14_2/test')
...
def process_heartbeat_dataset():
    """
    Process heartbeat dataset and save features to CSV files.
    """
    ...
    print("=== 심장박동 오디오 세그먼트 웨이블릿 특징 추출 ===")
    ...
    for filename in sorted(wav_files):
        ...
        try:
            ...
            if segment_count > 0:
                ...
                print(f"  추출된 세그먼트: {segment_count}개")
            
            processed_files += 1
            
            if processed_files % 10 == 0:
                print(f"\n진행 상황: {processed_files}/{len(wav_files)} 파일 처리 완료 ({total_segments} 세그먼트)")
    ...
    if all_data:
        ...
        csv_filename = f'wavelet_30_features_segments_{SEGMENT_LENGTH}_{TRAINING_DIR}.csv'
        ...
        desc_filename = f'feature_description_segments_{SEGMENT_LENGTH}.txt'
        feature_desc_path = os.path.join(SAVE_DIR, desc_filename)
        with open(feature_desc_path, 'w', encoding='utf-8') as f:
            f.write(f"30 Wavelet Features Description (Segment Length: {SEGMENT_LENGTH})\n")
            f.write("=" * 70 + "\n\n")
