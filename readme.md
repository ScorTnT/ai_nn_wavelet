...
현재 **5가지 특징 추출 방법**을 통한 웨이블릿 전처리 변환 후 모델 학습 및 성능 비교를 진행하고 있습니다:
...
- **데이터 구성**: 
   - `Train/` : 훈련 데이터 세트 2939 (라벨 정보: 0=정상, 1=비정상)
   - `Test/` : 테스트 데이터 세트 301 (라벨 정보: 0=정상, 1=비정상)
  - `training-a/` : 409
  - `training-b/` : 490
  - `training-c/` : 31
  - `training-d/` : 55
...
- **데이터 형식**: 
  - `.wav` 파일 (심장소리 녹음)
  - `REFERENCE.csv` (라벨 정보: -1=정상, 1=비정상) (라벨 정보 다른 것 주의**)
...
## 프로젝트 목적
- 웨이블릿 변환을 통한 심장 소리의 특징 추출 및 분류 성능 향상
- 다양한 머신러닝 및 딥러닝 모델을 사용하여 분류 성능 비교
- 웨이블릿 변환의 효과를 검증하고, 최적의 특징 추출 방법론 개발
...
│   ├── v6/                  # V6: 개선 버전
│   └── v7_features/         # V7: 30 차원 특징 벡터 최종본
                             ( 추후 v8 60 차원 특징 벡터 예정될 수 있음 (성능 향상에 따름) )
├── wavelet_/                # 전처리된 웨이블릿 데이터 (v1)
├── wavelet_v3/              # 전처리된 웨이블릿 데이터 (v3)
├── wavelet_v4/              # 전처리된 웨이블릿 데이터 (v4)
├── wavelet_v5/              # 전처리된 웨이블릿 데이터 (v5)
├── wavelet_v6/              # 전처리된 웨이블릿 데이터 (v6)
├── wavelet_v7_features/     # V7: 30 차원 특징 벡터
│       └── v2/              # v2: V7 개선 버전
│       └── v3/              # v3: 파일 이름 제외 포함
├── source/                  # 관련 소스 코드 및 연구
└── papers/                  # 관련 연구 논문들
...
### 모델 아키텍처 (연구중)
- **입력층**: 웨이블릿 특징 벡터 input_shape=(30,) (code/main)
- **은닉층**: 
...
- [x] **5레벨 웨이블릿 분해**: 다중 해상도 특징 추출
- [x] **30차원 특징 벡터**: 5가지 특징 × 5레벨 최적화

### 진행 중인 작업
- [ ] **main 코드**: 30차원 특징 벡터 기반 코드 작성 및 모델 학습
- [ ] **성능 비교**: 기존 10차원 vs 새로운 30차원 특징
- [ ] **데이터 전처리 연습**: 다양한 특징 추출 방법론 실험

### 향후 계획
- [ ] **V8**: 60차원 특징 벡터 연구 (성능 향상 여부에 따라)
- [ ] **성능 비교**: 새로운 30차원 특징 vs 새로운 60차원 특징
...
### 모델 훈련
```bash
cd /workspace/code/main
python train_model.py --features /workspace/wavelet_v7_features/v2
```
